						...
						function getCourses($accountID, $tokenHeader){
							...
							// To handle paginated results we will loop through these commands a number of times
								...
								//for ($k=0; $k<1; $k++){
									...
									//$accountCourses = listAccountCourses($accountID, $additionalParams, 1, $tokenHeader);
									$workingList = json_decode($accountCourses,true);

									
									//$GLOBALS['jsonData'] .= ' {" listAccountCourses: "' . $accountCourses . ' },';
									//$GLOBALS['jsonData'] .= ' {" accountCourses: "' . print_r($accountCourses,true) . ' },';
									//$GLOBALS['jsonData'] .= ' {" $workingList: "' . print_r($workingList,true) . ' },';
